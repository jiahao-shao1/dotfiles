...
Uses **Google Gemini API** (`gemini-3-pro-image-preview`) via the `generate_image.py` script.

**Script location**: `~/.openclaw/skills/nano-banana-pro/scripts/generate_image.py`

**Prerequisites**:
- `uv` installed (for dependency management)
- `GEMINI_API_KEY` environment variable set, or pass `--api-key` flag

**Steps**:

1. **Check for existing file**: Before generating, check if `infographic.png` exists
   - If exists: Rename to `infographic-backup-YYYYMMDD-HHMMSS.png`
2. **Read the prompt file** (`prompts/infographic.md`) and pass its full content as the `--prompt` argument
3. **Map aspect ratio to resolution**:
   - landscape (16:9) / square (1:1) → `1K` (default)
   - portrait (9:16) for high-density content → `2K` or `4K` as needed
4. **Run the script**:

```bash
uv run ~/.openclaw/skills/nano-banana-pro/scripts/generate_image.py \
  --prompt "$(cat prompts/infographic.md)" \
  --filename infographic.png \
  --resolution 1K
```

5. **On failure**, auto-retry once. If still fails, report the error to the user.
6. **Verify output**: Confirm `infographic.png` was created and display it to the user.
