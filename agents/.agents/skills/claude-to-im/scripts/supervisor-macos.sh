...
build_env_dict() {
  ...
  for var in HOME PATH USER SHELL LANG TMPDIR HTTPS_PROXY HTTP_PROXY ALL_PROXY https_proxy http_proxy all_proxy NO_PROXY no_proxy; do
    local val="${!var:-}"
    [ -z "$val" ] && continue
...
<dict>
    ...
    <array>
        ...
        <string>${SKILL_DIR}/scripts/proxy-preload.mjs</string>
