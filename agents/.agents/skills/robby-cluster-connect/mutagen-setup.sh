#!/bin/bash
# mutagen-setup.sh — Install mutagen file sync through ssctl SSH proxy
#
# ssctl's Go-based SSH proxy has two issues that break mutagen:
...
#
# Examples:
#   bash mutagen-setup.sh h20 ~/repo/my_project /root/my_project
#   bash mutagen-setup.sh h20 ~/repo/my_project /root/my_project my-sync output wandb data
#
# Prerequisites:
#   1. mutagen installed (brew install mutagen)
#   2. SSH tunnel active (ssctl)
...
Examples:
  bash mutagen-setup.sh h20 ~/repo/hatssl /root/hatssl
  bash mutagen-setup.sh h20 ~/repo/hatssl /robby/share/3D/xn/hatssl my-sync output wandb data
...
# ssctl doesn't close connections, so we kill SSH after the command finishes.
# The sentinel pattern (___DONE___) tells us when output is complete.
remote_exec() {
    local cmd="$1"
    local timeout="${2:-10}"
    local output
...
if echo "$REMOTE_CHECK" | grep -q "${MUTAGEN_VERSION}"; then
    ...
    echo "    This may take 30-60s through ssctl..."
...
# ssctl merges stderr into stdout, which corrupts mutagen's binary protocol.
# The wrapper redirects agent stderr to a log file.
echo "==> Installing stderr wrapper..."
WRAPPER_SCRIPT="#!/bin/bash
exec ~/$(dirname "$AGENT_PATH")/mutagen-agent-real \"\\\$@\" 2>/tmp/mutagen-agent.log"
